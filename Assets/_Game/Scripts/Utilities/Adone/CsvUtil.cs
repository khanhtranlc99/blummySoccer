///source: https://github.com/sinbad/UnityCsvUtil
using UnityEngine;
...
namespace Sinbad
{
    ...
    public static class CsvUtil
    {
        ...
        public static U[] LoadAllResourceObjects<U, T>(string filename, bool strict = true) where T : new() where U : List<T>
        {
            var b = UnityEngine.Resources.LoadAll<TextAsset>(filename);
            if (b == null || b.Length == 0)
                return null;
            return Array.ConvertAll(b, x =>
            {
                using (var stream = new MemoryStream(x.bytes))
                {
                    using (var rdr = new StreamReader(stream))
                    {
                        return LoadObjects<U, T>(rdr, strict);
                    }
                }
            });
        }

        public static U LoadAssetObjects<U, T>(string filename, bool strict = true) where T : new() where U : List<T>
        {
#if UNITY_EDITOR
            var a = $"Assets/_IdleTd/{filename}.csv";
            try
            {
                var b = ((TextAsset)UnityEditor.AssetDatabase.LoadAssetAtPath(a, typeof(TextAsset))).bytes;
...
#else
            return LoadResourceObjects<U, T>(filename, strict);
#endif
        }

        public static U LoadResourceObjects<U, T>(string filename, bool strict = true) where T : new() where U : List<T>
        {
            var b = UnityEngine.Resources.Load<TextAsset>(filename).bytes;
            using (var stream = new MemoryStream(b))
            {
                using (var rdr = new StreamReader(stream))
                {
        ...
        public static List<T> LoadObjects<T>(TextReader rdr, bool strict = true) where T : new()
        {
            ...
            string line;
            while (string.IsNullOrEmpty((line = rdr.ReadLine())) == false)
            {
                var obj = new T();
        ...
        // @param strict If true, log errors if a line doesn't have enough
        //   fields as per the header. If false, ignores and just fills what it can
        public static U LoadObjects<U, T>(TextReader rdr, bool strict = true) where T : new() where U : List<T>
        {
            var ret = (U)Activator.CreateInstance(typeof(U));
            string header = rdr.ReadLine();
            var fieldDefs = ParseHeader(header);
            FieldInfo[] fi = typeof(T).GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            PropertyInfo[] pi = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        private static void WriteObjectToLine<T>(T obj, FieldInfo[] fi, TextWriter w)
        {
            ...
            foreach (FieldInfo f in fi)
            {
                ...
                var objVal = f.GetValue(obj);
                if (objVal == null)
                    Debug.LogError($"objval null {f.Name}");
                string val = f.GetValue(obj).ToString();
                if (val == null)
                    Debug.LogError($"null val {f.Name}");
        ...
        private static bool SetField(string fieldName, string val, FieldInfo[] fi, PropertyInfo[] pi, object destObject)
        {
            ...
            foreach (FieldInfo f in fi)
            {
                ...
                {
                    Debug.LogError($"csv missing! cot: {fieldName} | gia tri: {val}");
                    return false;
                }
