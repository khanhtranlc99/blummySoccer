...
let XGameUnityAudio = {
  ...
  onAudioSourceDownloadSuccess: function (param_url) {
    ...
    if (this._clips.has(param_url)) {
      ...
      if (clip.context == null) {
        ...
        //进行创建
        var context = wx.createInnerAudioContext({ useWebAudioImplement: !clip.isLong });
  ...
  getAudioCachePath: function (url) {
    ...
    var result = `${wx.env.USER_DATA_PATH}/${hash}`;
  ...
  noticeUnity: function (method, msg) {
    window.unityNamespace.Module.SendMessage("_XGAME_H5_AUDIO", method, msg);
  },
  //获取缓存大小
  getCacheSize: function () {
    ...
    var size=0;
    let fs = wx.getFileSystemManager()
    var stats = fs.statSync(wx.env.USER_DATA_PATH, true);
    var prefix = `/_xgua_`;
  ...
  clearCacheAudioFile: function (ignores) {
    ...
    stats.forEach(element => {    
      ...
      if(fPath.startsWith(prefix)){
        ...
        var fullPath=`${wx.env.USER_DATA_PATH}${fPath}`;
...
console.log("xgame-unity-audio-wechat load done");
