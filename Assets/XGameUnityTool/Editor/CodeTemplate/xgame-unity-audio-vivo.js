...
let XGameUnityAudio = {
  ...
  fileExists: function (params) {
    try {
      var res = qg.accessFile({ uri: params });
      if (res == "true") {
        return true;
      }
    } catch (e) {
  ...
  //获取缓存大小
  getCacheSize: function (call_back_id) {
    this.xgua_log("getCacheSize");
    var result = 0;
    var prefix = `${qg.env.USER_DATA_PATH}/_xgua_`;
    qg.listDir({
      uri: `${qg.env.USER_DATA_PATH}/`,
      success: (res) => {
        var list = res.fileList;
        for (const element of list) {
          this.xgua_log("element-path:", element.uri, "  size:", element.length);
          if (element.uri.startsWith(prefix)) {
            result += element.length;
          }
        }
        var msg = JSON.stringify({ call_back_id: call_back_id, size: result });
        //触发完成回调
        this.noticeUnity("OnGetCacheSizeCallBack", msg)

      },
      fail: (data, code) => {
        console.log(`getCacheSize listDir fail, code = ${code}`)
        this.xgua_log(data, code);
        //触发失败回调
        var msg = JSON.stringify({ call_back_id: call_back_id, size: result });
        //触发完成回调
        this.noticeUnity("OnGetCacheSizeCallBack", msg)
  ...
  clearCacheAudioFile: function (ignores) {
    ...
    qg.listDir({
      ...
      success: (res) => {
        var list = res.fileList;
        for (const element of list) {
          var fPath = element.uri;
          if (fPath.startsWith(prefix)) {
            this.xgua_log("fullPath:", fPath);
            if (!map.has(fPath)) {
              this.xgua_log("删除:", fPath);
              qg.deleteFile({
                uri: fPath,
                ...
                fail: function (data, code) {
                  console.log(`deleteFile fail, code = ${code}`)
                }
              });
            }
          }
      ...
      fail: (data, code) => {
        console.log(`listDir fail, code = ${code}`)
      }
    });
  }
}
...
window["XGameH5Audio_CallMethod"] = (method, json) => {
  ...
  switch (method) {
    ...
    case METHOD_ID.STOP_BGM:
      ...
      break;
    case METHOD_ID.GET_CACHE_SIZE:
      {
        var data = JSON.parse(json);
        XGameUnityAudio.getCacheSize(data[STRINGS.call_back_id]);
      }
      break;
...
window["XGameH5Audio_GetCacheSize"] = () => {
  ...
  console.log("此方法无效！请使用XGameH5Audio_CallMethod，调用异步方法")
  return 0;
};

console.log("xgame-unity-audio-vivo load done");
