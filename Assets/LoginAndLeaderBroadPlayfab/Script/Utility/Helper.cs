...
public static class Helper
{
    ...
    public static IEnumerator StartOnNextFrame(Action action)
    {
    ...
    public async static void GetCountryByIP()
    {
        ...
        using (var client = new WebClient())
        {
            ...
            UseProfile.FlagLink = string.Format(StringHelper.FLAG_API, ipInfo.country);
    ...
    public static DateTime ParseTime(string time, DateTime defaultValue = default, IFormatProvider cultureInfo = null, DateTimeStyles styles = DateTimeStyles.None)
    {
        if (!DateTime.TryParse(time, cultureInfo ?? CultureInfo.InvariantCulture, DateTimeStyles.None, out var result))
        {
            result = (defaultValue == default) || string.IsNullOrEmpty(time) ? UnbiasedTime.Instance.Now() : defaultValue;
        }
        return result;
    }
    public static void ParseTime(string time, out DateTime result, DateTime defaultValue = default, IFormatProvider cultureInfo = null, DateTimeStyles styles = DateTimeStyles.None)
    {
        if (!DateTime.TryParse(time, cultureInfo ?? CultureInfo.InvariantCulture, DateTimeStyles.None, out result))
        {
            result = (defaultValue == default) || string.IsNullOrEmpty(time)  ? UnbiasedTime.Instance.Now() : defaultValue;
        }
    }
