...
{
    [Serializable, TweenAnimation("Rendering/Clip From Spline", "Clip From Spline")]
    public class TweenClipFromSpline : TweenFloat<SplineRenderer>
    {
        public override float current
        {
            get => target ? (float)target.clipFrom : 1f;
            set { if (target) target.clipFrom = value; }
        }

    }
    [Serializable, TweenAnimation("Rendering/Spline size", "Spline size")]
    public class TweensSplineRender : TweenFloat<SplineRenderer>
    {
        public override float current
        {
            get => target ? (float)target.size : 1f;
