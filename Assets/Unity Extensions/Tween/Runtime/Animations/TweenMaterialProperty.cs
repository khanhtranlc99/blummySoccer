...
{
    [Serializable, TweenAnimation("Rendering/Shared Material Property", "Shared Material Property")]
    public class TweenSharedMaterialProperty : TweenFloat<Material>
    {
        ...
        public override float current
        {
            get
            {
                if (target)
                {
                    return target.GetFloat(PropertyFieldName);
                }
            ...
            set
            {
                if (target)
                {
                    target.SetFloat(PropertyFieldName, value);
                }
            }
        }
    }
    ...
    public class TweenMaterialProperty : TweenFromTo<Vector4>
    {
        ...
        public override void RestoreState()
        {
            if (_originalTarget)
            {
                ...
                target.SetPropertyBlock(_tempBlocks[_tempBlocks.Count - 1].isEmpty
                    ? null
                    : _tempBlocks[_tempBlocks.Count - 1]);
        ...
        protected override void OnPropertiesGUI(TweenPlayer player, SerializedProperty property)
        {
            ...
            switch (propertyType)
            {
                ...
                case Type.Range:
                    ...
                    FromToFieldLayout("Value", fromProp.FindPropertyRelative(nameof(Vector4.x)),
                        toProp.FindPropertyRelative(nameof(Vector4.x)), fromDisable);
                    break;

                case Type.Vector:
                    FromToFieldLayout("X", fromProp.FindPropertyRelative(nameof(Vector4.x)),
                        toProp.FindPropertyRelative(nameof(Vector4.x)), fromDisable);
                    FromToFieldLayout("Y", fromProp.FindPropertyRelative(nameof(Vector4.y)),
                        toProp.FindPropertyRelative(nameof(Vector4.y)), fromDisable);
                    FromToFieldLayout("Z", fromProp.FindPropertyRelative(nameof(Vector4.z)),
                        toProp.FindPropertyRelative(nameof(Vector4.z)), fromDisable);
                    FromToFieldLayout("W", fromProp.FindPropertyRelative(nameof(Vector4.w)),
                        toProp.FindPropertyRelative(nameof(Vector4.w)), fromDisable);
                    break;
                ...
                case Type.Color:
                    ...
                    var fromRect = new Rect(rect.x + labelWidth, rect.y, (rect.width - labelWidth - 8) / 2,
                        rect.height);
                    ...
                    using (LabelWidthScope.New(12))
                    {
                        ...
                        fromProp.vector4Value = EditorGUI.ColorField(fromRect, EditorGUIUtilities.TempContent("F"),
                            fromProp.vector4Value, false, true, true);
                        EditorGUI.EndDisabledGroup();
                        toProp.vector4Value = EditorGUI.ColorField(toRect, EditorGUIUtilities.TempContent("T"),
                            toProp.vector4Value, false, true, true);
                    }
